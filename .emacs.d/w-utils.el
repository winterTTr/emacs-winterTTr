...
;; some const variable
(defconst w/os:windowsp (eq system-type 'windows-nt)
...
(defconst w/os:win64p (and w/os:windowsp
                              (getenv "PROGRAMW6432"))
  "if current operation system is windows 64bit verison.")

(defconst w/host:MSWSp (string-equal system-name "WTTR-T5500")
...
(defun w/prepend-to-exec-path (path)
...
(defun w/log:message (message &rest args)
...
(defun w/prepend-to-load-path (path)
  "prepend the PATH to the head of the `load-path', return updated load-path."
  (add-to-list 'load-path path))


(defun w/plugin:expand-file-name (relative-path &optional base-diretory)
  "Expand the RELATIVE-PATH according to BASE-DIRECORY.
If BASE-DIRECORY is nil, use `w/plugin:base-directory'."
  (expand-file-name relative-path (or base-diretory w/plugin:base-directory)))


(defun w/plugin:prepend-to-load-path (plugin-folder-name &optional base-diretory)
  "Prepend the plugin in BASE-DIRECTORY in the `load-path', so that you can use `require' later.
If BASE-DIRECTORY is nil, use `w/plugin:base-directory' as base director.
This function will also check if the plugin folder exist before prepend to `load-path'."
  (let ((full-path (w/plugin:expand-file-name plugin-folder-name base-diretory)))
    (cond
     ((file-exists-p full-path)
      (w/prepend-to-load-path full-path)
      full-path)
     (t
      (w/log:message "plugin dir [%s] does not exist, loading possibly failed." full-path)
      nil))))


(defun w/delete-trailing-whitespace-when-save ()
...
(defun w/kill-buffer-may-have-clients ()
...
(when w/os:windowsp
  (defun w/w32-restore-frame ()
    "Restore a minimized frame"
    (interactive)
    (w32-send-sys-command 61728))

  (defun w/w32-maximize-frame ()
